...
class PlayingGameState {
  ...
  newFood() {
    let snakeIndexes: number[] = this.snake.positions.map(pos => pos.asIndex());

    // Start with the list of possible indexes
    let availablePosIndexes: number[] = []
    for (let i = 0; i < 42; i++) {
...
class GameOverScene implements Scene<GameOverState, SnakeControllerInput> {
  ...
  initialRender(driver: ConnectBoardDriver) {
    console.debug('[GameOverScene] initialRender not implemented')
  }
